...
class CycleGAN:
    ...
    def get_batch_data(self,data,idx):
        ...
        # Randomly flip remember to change back
    ...
    def setup(self):
        # change size of placeholder in accordance with bach size (1 for testing, other for training)
        self.real_a = tf.placeholder(tf.float32, 
                                     [1, self.w, self.h, self.c],
                                     name='input_a')
        self.real_b = tf.placeholder(tf.float32, 
                                     [1, self.ow, self.oh, self.oc],
                                     name='input_b')
        ...
        self.X1 = tf.placeholder(dtype=tf.float32, shape=[1, 64, 64, 1])
        self.n_fake_a = 0
        self.n_fake_b = 0
  ...
  #          maxval=1
    ...
    def train(self,data,label):
        ...
        input_data = data
        #input_data = np.expand_dims(px, axis=3)
